...
function go_next() {
	if(document.formm.okon1[0].checked == true){//동의함이 선택된 경우
		...
		alert("약관에 동의하셔야 회원가입 진행이 가능합니다.");
	}
...
function idcheck(){
	if (document.getElementById("id").value==""){
		alert("아이디를 입력해주세요.");
	document.getElementById("id").focus();//입력대기
	
	return false;
	}
	
	//ID 중복확인 화면 요청
	// URL - Controller 요청 URL
	//_blank_ -새로운 윈도우 생성하여 표시
	var url = "id_check_form?id="+ document.getElementById("id").value;
	window.open(url, "_blank_","toolbar=no,menubar=no,scrollbars=yes, resizeable=no,width=450,height=350");
...
function post_zip(){
	...
	window.open(url, "_blank_","toolbar=no,menubar=no,scrollbars=yes, resizeable=no,width=700,height=500");	
}
...
function find_id_form(){
	...
	window.open(url, "_blank_","toolbar=no,menubar=no,scrollbars=yes, resizeable=no,width=550,height=450");	
}
